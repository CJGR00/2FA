...
async function createSession(userId, req, res) {
  ...
  res.cookie('sessionToken', plainToken, {
    httpOnly: true,
    secure: secureFlag,
    sameSite: 'strict',
...
app.post('/api/login', authLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!user.two_fa_enabled || user.role === 'admin') {
      ...
      const token = await createSession(user.user_id, req, res);
      return res.json({
...
// 3. Resend 2FA Code
app.post('/api/2fa/resend', authLimiter, async (req, res) => {
  ...
  try {
    ...
    if (preAuthSession.method === 'Email') {
      const code = security.generateEmailCode();
      const codeHash = security.hashSHA256(code);
      const codeExpiry = Date.now() + (parseInt(process.env.CODE_EXPIRATION_MINUTES || '5', 10) * 60 * 1000);
  ...
  try {
    ...
    if (cleanCode.includes('-') && cleanCode.length === 9) {
      ...
      if (matchedIndex !== -1) {
        ...
        const sessionToken = await createSession(user.user_id, req, res);
        return res.json({
    ...
    // ----------------------------------------------------
    // Scenario C: Email code (requires database matching)
    // ----------------------------------------------------
    if (preAuthSession.method === 'Email') {
      const hashedInputCode = security.hashSHA256(cleanCode);
...
// 6. 2FA Setup Trigger Verification Code (For Email verification setup before enabling)
...
app.put('/api/admin/users/:id', authenticateSession, requireAdmin, async (req, res) => {
  ...
  try {
    const existingUser = await dbQuery.get('SELECT user_id FROM users WHERE user_id = ?', [id]);
    if (!existingUser) {
      return res.status(404).json({ error: 'User not found.' });
    }

    if (username) {
      const usernameTaken = await dbQuery.get(
...
initDatabase()
  .then(async () => {
    ...
    app.listen(PORT, () => {
      console.log(`==================================================`);
      console.log(`TrustFactor running on: http://localhost:${PORT}`);
      console.log(`==================================================`);
    });
  })
  .catch((err) => {
